...
import com.wllfengshu.jmj.provider.player.constant.CommonConstant;
...
public class RegisterBiz extends AbstractInterfaceBase<RegisterRequest, RegisterResponse> {
    ...
    protected RegisterResponse doProcess(RegisterRequest request) {
        ...
        if (!StringUtils.equals(request.getPassword(), request.getConfirmPassword())) {
            ...
            response.setBizResponseCode(CommonEnum.THE_TWO_PASSWORDS_ARE_INCONSISTENT.getCode());
            response.setBizResponseDesc(CommonEnum.THE_TWO_PASSWORDS_ARE_INCONSISTENT.getDesc());
            return response;
        }
        // 2查询username是否已经存在
        ...
        if (!CollectionUtils.isEmpty(oldPlayer)) {
            RegisterResponse response = new RegisterResponse();
            response.setBizResponseCode(CommonEnum.USER_EXISTS.getCode());
            response.setBizResponseDesc(CommonEnum.USER_EXISTS.getDesc());
            return response;
        }
        // 3save
        ...
        po.setHeadPortraitUrl(StringUtils.defaultIfEmpty(request.getHeadPortraitUrl(), CommonConstant.REGISTER_HEAD_PORTRAIT_URL));
        int affNumber = playerDAO.insert(po);
        if (NumberUtils.INTEGER_ONE != affNumber) {
            RegisterResponse response = new RegisterResponse();
            response.setBizResponseCode(CommonEnum.SAVE_USER_FAILED.getCode());
            response.setBizResponseDesc(CommonEnum.SAVE_USER_FAILED.getDesc());
            return response;
        }
        // 4success
        RegisterResponse response = new RegisterResponse();
